...
>
> Стоит отметить, что запуск проекта в локальном режиме по-прежнему возможен, но требует наличия доступной PostgreSQL базы локально или удалённо, и ручного запуска Celery, если он нужен, или переменной `DISABLE_CELERY=1` в `.env.local` для его отключения 
